...
### 陷阱 1: 自动假设论文 ⚠️ HIGH-SEVERITY
当用户说"审核这篇论文"但没有指定文件路径或论文标题时，**永远不要假设是哪篇论文**。用户的工作目录中可能有多篇论文（自己的投稿、要审的论文、参考论文等）。

**错误做法**: 看到用户有 ACML 论文，就自动审它。
**正确做法**: 使用 clarify 问清楚"你要审核哪篇论文？请提供文件路径或论文标题。"

### 陷阱 2: GitHub 推送是工作流的一部分
用户期望审核报告 + skill 本身都被推送到 GitHub。skill 创建完成后不是终点——还需要推送到用户仓库（TheeTarnished）。

**完整工作流应该是**:
1. 创建/更新 skill
2. 审核论文
3. 将 skill + 审核报告推送到用户的 GitHub 仓库
4. 确认推送成功

### 陷阱 3: 不要混淆"用户的论文"和"要审的论文"
用户的 ACML 2026 论文 (acml26_design_space.tex) 是用户自己的投稿。除非用户明确说"审一下我的 ACML 论文"，否则不要审它。用户更可能有其他目标论文（如经典论文 ResNet、竞品论文等）。
